...
def createGraph(ax,monthNum,playType,classroom):
    ...
    for date_prefix in date_range:
        ...
        filtered_df = df[(df['type'] == playType) & 
    ...
    sns.lineplot(
        ...
        markers=True,
        dashes=False,
        palette=['#FFFFFF', '#1A2B55'],
        markeredgecolor='#010101',
        linewidth=2.5
    )
    ...
    ax.text(0.12, 1.06, ' ', transform=ax.transAxes, fontsize=1, bbox=dict(facecolor='#FFFFFF', alpha=1))
    ax.text(0.7, 1.05, 'This School Year', transform=ax.transAxes, fontsize=10)
    ax.text(0.66, 1.06, ' ', transform=ax.transAxes, fontsize=1, bbox=dict(facecolor='#1A2B55', alpha=1))
    

    # Set the labels and title
    ax.set_title('YEAR-OVER-YEAR • ' + playType, fontsize=16, weight='bold',color = '#1A2B55',pad=25)
